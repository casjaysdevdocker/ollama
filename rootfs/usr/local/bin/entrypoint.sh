...
#SERVICE_USER="${SERVICE_USER:-ollama}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-ollama}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    if [ "$ENTRYPOINT_MESSAGE" = "yes" ]; then
      echo "Executing entrypoint script for ollama"
    fi
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
