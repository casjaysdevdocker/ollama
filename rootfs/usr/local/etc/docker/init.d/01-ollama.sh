...
# @@License          :  WTFPL
# @@ReadME           :  01-ollama.sh --help
# @@Copyright        :  Copyright: (c) 2026 Jason Hempstead, Casjays Developments
# @@Created          :  Wednesday, Jan 28, 2026 23:21 EST
# @@File             :  01-ollama.sh
# @@Description      :  
...
if [ -n "$OLLAMA_APPNAME_ENABLED" ]; then
  if [ "$OLLAMA_APPNAME_ENABLED" != "yes" ]; then
...
RUN_DIR="/run/ollama"
# set log directory
LOG_DIR="/data/logs/ollama"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the working dir
WORK_DIR="/tmp/ollama"
# - - - - - - - - - - - - - - - - - - - - - - - - - -
# port which service is listening on
SERVICE_PORT="${OLLAMA_PORT:-11434}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# root user name
root_user_name="${OLLAMA_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${OLLAMA_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${OLLAMA_USER_NAME:-}"
# normal user password
user_pass="${OLLAMA_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
if [ -f "/config/env/ollama.script.sh" ]; then
  . "/config/env/ollama.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/ollama.sh" ]; then
  . "/config/env/ollama.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__execute_prerun() {
  ...
  if [ -z "$OLLAMA_NUM_THREADS" ]; then
    ...
    echo "🚀 CPU Optimization: Using $cpu_cores cores with OpenBLAS"
  fi
  
  # GPU detection and logging (Ollama handles GPU automatically)
  if [ -d "/dev/dri" ] || [ -c "/dev/nvidiactl" ] || [ -c "/dev/kfd" ]; then
    echo "🎮 GPU detected - Ollama will use GPU acceleration if available"
  else
    echo "💻 CPU mode - Using optimized OpenBLAS (expect 3-5x faster inference)"
  fi
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/ollama.conf"
...
__post_execute() {
  ...
  (
    ...
    # Auto-pull models if MODELS env variable is set
    # Default to qwen2.5-coder:3b if MODELS not specified (best for general coding on CPU)
    local MODELS_TO_PULL="${MODELS:-qwen2.5-coder:3b}"
    
    if [ -n "${MODELS_TO_PULL}" ]; then
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      echo "📦 Auto-pulling Ollama models"
      [ -z "${MODELS}" ] && echo "   Using default: qwen2.5-coder:3b (optimal for CPU coding)"
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      
      # Parse MODELS variable - support comma, space, or mixed delimiters
      # Convert commas to spaces first, then normalize whitespace
      MODELS_NORMALIZED=$(echo "${MODELS_TO_PULL}" | tr ',' ' ' | tr -s ' ')
      
      # Convert to array
      IFS=' ' read -ra MODELS_ARRAY <<< "$MODELS_NORMALIZED"
      
      # Get first model as default
      DEFAULT_MODEL="${MODELS_ARRAY[0]}"
      TOTAL_MODELS="${#MODELS_ARRAY[@]}"
      
      echo "🎯 Default model: ${DEFAULT_MODEL}"
      echo "📊 Total models to pull: ${TOTAL_MODELS}"
      echo ""
      ...
      echo "✓ Successfully pulled: ${PULLED_COUNT}/${TOTAL_MODELS}"
      if [ ${FAILED_COUNT} -gt 0 ]; then
        echo "✗ Failed: ${FAILED_COUNT}/${TOTAL_MODELS}"
      fi
      echo "🚀 Default model: ${DEFAULT_MODEL}"
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    fi
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$OLLAMA_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$OLLAMA_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$OLLAMA_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$OLLAMA_USER_PASS_WORD}"             #
